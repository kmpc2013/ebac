...
def test_calcular_soma(mocker):
    ...
    assert response.json() == {"task_id": "fake-task-id", "message": "Tarefa de soma enviada para execução!"}
...
def test_calcular_fatorial(mocker):
    mock_somar_delay = mocker.patch("tasks.fatorial.delay")
    mock_redis_lpush = mocker.patch("main.redis_client.lpush")
    mock_redis_ltrim = mocker.patch("main.redis_client.ltrim")

    mock_somar_delay.return_value.id = "fake-task-id"

    response = client.post("/calcular/fatorial", params={"n": 4})

    assert response.status_code == 200
    assert response.json() == {"task_id": "fake-task-id", "message": "Tarefa de fatorial enviada para execução!"}
